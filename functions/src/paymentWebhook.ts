...
const ICICI_SECRET_KEY = defineSecret("ICICI_PG_SECRET_KEY");
...
const ICICI_MERCHANT_ID = defineString("ICICI_MERCHANT_ID");
...
interface ParsedAdvice {
    ...
    txnID: string; // ICICI's unique ref number for this transaction — used for idempotency
    paymentID: string | null; // the authorization/payment id generated by ICICI's auth system
    status: PaymentStatus;
    amount: string | null;
}

/** Hash Calc V1: sort field names, concatenate values, HMAC-SHA256, hex, lowercase. */
function computeSecureHashV1(fields: Record<string, unknown>, secretKey: string): string {
...
function verifySignature(req: Request, secretKey: string): boolean {
    ...
    if (contentType.includes("application/json")) {
        ...
        if (!headerHash || Array.isArray(headerHash)) return false;
    ...
    const providedHash = req.body?.secureHash;
...
function parseAdvice(body: Record<string, unknown>): ParsedAdvice | null {
    const merchantTxnNo = body.merchantTxnNo;
    const txnID = body.txnID;
    if (!merchantTxnNo || !txnID) return null;
    ...
    if (responseCode === "R1000") return null;

    const status: PaymentStatus = responseCode === "000" || responseCode === "0000" ? "success" : "failure";
...
async function resolveTargetDoc(merchantTxnNo: string): Promise<FirebaseFirestore.DocumentReference | null> {
    const orderSnap = await db.collection("paymentOrders").doc(merchantTxnNo).get();
    if (!orderSnap.exists) return null;

    const order = orderSnap.data() as { targetCollection?: string; targetDocId?: string };
    if (!order.targetCollection || !order.targetDocId) return null;
    if (!ALLOWED_COLLECTIONS.has(order.targetCollection)) return null;

    return db.doc(`${order.targetCollection}/${order.targetDocId}`);
}

export const paymentWebhook = onRequest(
    {secrets: [ICICI_SECRET_KEY], region: "us-central1"},
    async (req: Request, res: Response) => {
        ...
        if (ICICI_MERCHANT_ID.value() && String(body.merchantId ?? "") !== ICICI_MERCHANT_ID.value()) {
