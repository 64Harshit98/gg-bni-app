...
const useOrdersData = (companyId?: string) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // Query orders collection under the company document, order by creation time descending
    // This matches your Firestore rules which expect tenant data under /companies/{companyId}/...
    const ordersQuery = query(
      collection(db, `companies/${companyId}/customerOrders`),
...
const OrdersPage: React.FC = () => {
  ...
  const [activeStatusTab, setActiveStatusTab] =
    useState<OrderStatus>('Upcoming'); // Default tab
  ...
  const {
    ...
    loading: dataLoading,
    error,
  } = useOrdersData(currentUser?.companyId);
  ...
  const handleUpdateStatus = async (
    ...
    const orderDocRef = doc(
      db,
      `companies/${currentUser?.companyId}/customerOrders`,
      orderId,
    );
  ...
  return (
    <div className="flex min-h-screen w-full flex-col overflow-hidden bg-gray-100 mb-10 ">
      ...
      {/* Status Tabs */}
      <div className="flex justify-center border-b border-gray-300 p-2 bg-white sticky top-[calc(4rem+1px)] z-10 grid grid-cols-2 gap-2">
        {' '}
