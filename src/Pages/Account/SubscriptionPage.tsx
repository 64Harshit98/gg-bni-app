    ...
    <div className="max-w-4xl mx-auto mb-8 transition-all duration-300">
      <div className="bg-white rounded-sm shadow-md border border-green-100 overflow-hidden">
        ...
        >
            ...
            <svg className={`w-6 h-6 text-white transition-transform duration-300 ${isDetailsOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-base font-semibold text-gray-800">
                            {expiryDate 
                            ? new Date((expiryDate as any).toDate ? (expiryDate as any).toDate() : expiryDate).toLocaleDateString()
                            : 'Lifetime / Unknown'}
                        </p>
                    </div>
                </div>
                    ...
                    <thead>
                        <tr>
                            ...
                            {currentTiers.map(tier => (
                                <th key={tier.id} className={`p-4 text-center border-b border-gray-200 relative ${tier.recommended ? 'bg-yellow-50/50' : 'bg-white'}`}>
                                    {tier.recommended && (
                                    ...
                                    <button 
                                        ...
                                        className={`mt-3 w-full py-1.5 rounded-sm text-xs sm:text-sm font-bold transition-colors ${
                                            tier.recommended 
                    ...
                    <tbody>
                        {allFeatures.map((feature, idx) => (
                            <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                                <td className="p-3 text-xs sm:text-sm font-medium text-gray-700 border-b border-gray-100 pl-4 sm:pl-8">
                                    {feature}
                                </td>
                                {currentTiers.map(tier => {
                                    const hasFeature = tier.features.includes(feature);
                                    return (
                                        <td key={tier.id} className="p-3 text-center border-b border-gray-100">
                                            {hasFeature ? (
