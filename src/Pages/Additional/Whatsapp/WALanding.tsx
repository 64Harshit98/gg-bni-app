...
const MessageQuotaPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-muted flex flex-col items-center justify-center">
      ...
      <div className="max-w-lg w-full bg-card rounded-xs shadow-2xl border border-border overflow-hidden">
        ...
        <div className="p-8">
          <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="p-5 bg-muted rounded-xs border flex flex-col items-center">
              <FiTrendingUp className="text-blue-500 mb-2" size={20} />
              <span className="text-[10px] text-muted-foreground uppercase font-black tracking-widest">Renewal</span>
              <span className="text-foreground font-bold text-xs">{quota?.refreshDate}</span>
            </div>
            ...
            >
              ...
              <span className="text-[10px] text-muted-foreground uppercase font-black tracking-widest mt-1">Status</span>
      ...
      {showQRModal && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-card w-full max-w-sm rounded-xs p-10 flex flex-col items-center relative shadow-2xl animate-in zoom-in-95">
            <button
            ...
            <h3 className="text-2xl font-black text-foreground mb-1">Link Device</h3>
            <p className="text-xs text-muted-foreground text-center mb-8 px-4">Open WhatsApp &gt; Linked Devices to scan this code.</p>

            <div className="relative w-64 h-64 bg-muted rounded-[2rem] flex items-center justify-center border-4 border-white shadow-inner overflow-hidden">
              {qrLoading && !qrCodeData ? (
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-xs text-muted-foreground font-bold mt-2">Connecting...</span>
                </div>
              )}
            </div>

            <div className="w-full mt-8 bg-muted px-6 py-4 rounded-2xl flex items-center justify-between border border-border">
              <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">New Code In</span>
