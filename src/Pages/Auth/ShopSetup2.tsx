...
import { Variant, ROLES, PLANS } from '../../enums';
...
const FinalSetupPage: React.FC = () => {
  ...
  const handleFinishSetup = async (e: React.FormEvent) => {
    ...
    try {
      // 1. DESTRUCTURE DATA
      // Extract specific objects to prevent them from landing in the wrong collection
      const {
        fullName, email, phoneNumber, password,
        initialStaff,
        pack,          // Plan Info
        salesSettings, // Settings from Step 3
        ...businessDetails // Remaining info (Name, Address, etc.)
      } = previousData;

      // 2. PREPARE PAYLOADS

      // A. Business Info (Clean profile data only)
      const businessInfoPayload = {
        ...businessDetails,
        createdAt: new Date(),
      };

      // B. Plan Payload (For Root Doc)
      const planPayload = {
        pack: pack || PLANS.PRO,
        validity: 'active',
      ...
      // C. Sales Settings Payload (For settings/sales-settings)
      const salesSettingsPayload = {
        ...(salesSettings || {}),
        salesViewType: salesViewType, // Add current view choice
        settingType: 'sales',
      };
      ...
      await registerUserWithDetails(
        ...
        password, 
        ROLES.OWNER, 
        businessInfoPayload, // Arg 6: Business Info
        planPayload,         // Arg 7: Plan Details
        salesSettingsPayload,// Arg 8: Sales Settings
        initialStaff || []   // Arg 9: Items (or Staff if you repurposed this)
      );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100 p-4">
      ...
      <form onSubmit={handleFinishSetup} className="flex flex-col flex-grow">
        ...
        {error && <div className="bg-red-50 text-red-600 p-4 rounded-lg text-sm text-center mb-4 border border-red-200 animate-pulse">Error: {error}</div>}
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-gray-100 border-t border-gray-200 z-50">
          <div className="max-w-md mx-auto space-y-4">
            <CustomButton type="submit" variant={Variant.Filled} disabled={isSubmitting} className="h-12 text-lg w-full">
              {isSubmitting ? <div className="flex items-center justify-center gap-2"><Spinner /><span>{statusMessage}</span></div> : <div className="flex items-center justify-center gap-2"><span>Complete Setup</span><FiCheckCircle /></div>}
            </CustomButton>
          </div>
        </div>
      </form>
