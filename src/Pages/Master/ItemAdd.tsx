...
const ItemAdd: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-gray-100 font-poppins text-gray-800 relative">
      ...
      {/* Regular error / info modal */}
      {form.modal && (
        <Modal message={form.modal.message} onClose={() => form.setModal(null)} type={form.modal.type} />
      )}

      {/* Overwrite confirmation modal */}
      {form.showOverwritePrompt && (
        <Modal
          message={`${form.pendingDuplicateCount} item${form.pendingDuplicateCount > 1 ? 's' : ''} with matching barcodes already exist. Overwrite them with the new data?`}
          onClose={form.handleCancelOverwrite}
          onConfirm={form.handleConfirmOverwrite}
      ...
      <div className="flex-1 flex flex-col md:flex-row relative">
        ...
        <div className="flex-1 w-full md:w-[65%] bg-gray-100 md:bg-gray-50 md:border-r border-gray-200 pt-28 pb-24 px-2 md:pt-6 md:px-6 md:pb-6 overflow-y-auto">
          ...
          {form.error && (
            <div className="mb-4 text-center p-3 bg-red-100 text-red-700 rounded-sm">{form.error}</div>
          )}
          ...
          <div className="md:hidden bg-white p-2 rounded-sm shadow-md mb-4">
            <div className="flex flex-col items-center justify-center mb-4">
              ...
              <input
                ...
                onChange={(e) => form.handleFileUpload(e, { skipFirstRow: true })}
                className="hidden"
                accept=".xlsx, .xls, .csv"
              />
              <button
          ...
          <div className="bg-white p-4 rounded-sm shadow-md md:mb-0 md:rounded-sm md:shadow-sm md:border md:border-gray-200 mb-10">
            ...
            {/* ── Dismissible success banner with auto-scroll target ── */}
            {form.success && (
              <div
                ref={form.successBannerRef}
                className="mb-4 p-3 bg-green-100 text-green-700 rounded-sm flex items-center justify-between gap-2"
              >
                <span className="flex-1 text-center">{form.success}</span>
                <button
                  onClick={() => form.setSuccess(null)}
