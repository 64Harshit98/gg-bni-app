    ...
    if (item.barcode) {
      ...
      try {
        JsBarcode(canvas, item.barcode, {
          format: 'CODE128',
          displayValue: true,        // show number below barcode like the real label
          fontSize: 10,
          textMargin: 2,
          height: 60,
          width: 2,
  ...
  const handlePrint = useCallback(async () => {
    ...
    try {
      ...
      for (const item of printQueue) {
        ...
        // Generate QR (Async)
        const qrDataUrl = await QRCodeLib.toDataURL(item.barcode, {
          width: 120, margin: 2,
    ...
    <div class="label">
        ...
        ${businessPhoneNumber ? `<div class="phone">${businessPhoneNumber}</div>` : ''}
        <img class="barcode-img" src="${barcodeDataUrl}" alt="barcode"/>
        <img class="qr-img" src="${qrDataUrl}" alt="qr"/>
        <div class="item-name">${item.name}${item.unit ? ` (${item.unit})` : ''}</div>
        <div class="item-mrp">MRP: &#8377;${item.mrp}</div>
    </div>
`;
        }
      }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="bg-gray-50 rounded-sm p-3 self-start border border-gray-100">
            ...
            >
              ...
              <span
                className={`text-xs text-gray-500 transition-transform duration-200 ${isPreviewOpen ? "rotate-180" : "rotate-0"
                  }`}
              >
                ▼
              </span>
          ...
          <div className="flex flex-col gap-3">
            ...
            {printQueue.map((item) => (
              <div
                ...
                className={`p-2 border rounded-sm bg-white flex items-center justify-between gap-2 cursor-pointer transition shadow-sm hover:shadow-md ${itemForPreview?.queueId === item.queueId
