...
const RestockReportPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(itemsQuery, (snapshot) => {
            const allItems: ItemDoc[] = snapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            } as ItemDoc));

            const filteredItems = allItems.filter(item =>
                item.restockQuantity > 0 && (item.stock || 0) <= item.restockQuantity
            );

            filteredItems.sort((a, b) =>
                ((a.stock || 0) - a.restockQuantity) - ((b.stock || 0) - b.restockQuantity)
            );
    ...
    const outOfStockCount = displayedItems.filter(i => (i.stock || 0) <= 0).length;
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen font-sans text-gray-800">
            ...
            <div className="flex items-center justify-between pb-3 border-b mb-2">
                <h1 className="flex-1 text-xl text-center font-bold text-gray-800">Restock Report</h1>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
                    <div>
                        <p className="text-sm font-medium text-gray-500">Critical (Out of Stock)</p>
                                ...
                                displayedItems.map((item) => {
                                    ...
                                    return (
                                        <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="p-4 text-center text-sm text-gray-500">{item.restockQuantity}</td>
