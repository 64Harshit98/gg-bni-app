...
const detectHeaderMap = (lines: string[]): HeaderMap | null => {
    for (const rawLine of lines) {
        ...
        if (keys.includes('QTY') && keys.includes('RATE') && keys.includes('AMOUNT') && keys.includes('DESCRIPTION')) {
            ...
            // HSN is often blank/non-numeric ("-") and its value is never consumed
            // downstream — requiring a numeric token for it was causing rows with
            // missing HSN to be skipped entirely. Drop it from the count-matching set.
            const leading = found.filter(f => f.index < descIndex && f.key !== 'HSN').map(f => f.key);
            const trailing = found.filter(f => f.index > descIndex && f.key !== 'HSN').map(f => f.key);
...
const parseItemsWithFixedPatterns = (lines: string[]): ParsedItem[] => {
    ...
    lines.forEach(rawLine => {
        ...
        // A real item name never contains a price like "500.00". If it does, the regex swallowed
        // neighbouring columns into the name, so drop the row instead of showing wrong data.
        if (/\d+\.\d{2}\b/.test(name)) return;
