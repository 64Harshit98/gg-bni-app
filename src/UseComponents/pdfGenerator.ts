...
export const generatePdf = async (data: InvoiceData, action: ACTION.DOWNLOAD | ACTION.PRINT | ACTION.BLOB = ACTION.DOWNLOAD): Promise<Blob | void> => {
  ...
  if (data.extraExpenseAmount && data.extraExpenseAmount > 0) {
    ...
    doc.text(`Add : ${data.extraExpenseName || 'Extra Expense'} (+)`, endX - 30, finalY + 4, { align: 'right' });
  ...
  if (billDiscount > 0) {
    ...
    doc.text('Less : Bill Discount (-)', endX - 30, finalY + 4, { align: 'right' });
  ...
  doc.text('Add : Rounded off (+)', endX - 30, finalY + 4, { align: 'right' });
