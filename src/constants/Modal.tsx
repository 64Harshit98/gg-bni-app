...
export const Modal: React.FC<ModalProps> = ({
    ...
    const modalContent = (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[2000] p-4">
            <div className="bg-card rounded-sm shadow-2xl p-6 w-full max-w-sm text-center">
                {/* Icon based on type */}
                ...
                {showConfirmButton ? (
                    <div className="flex gap-3">
                        <button
                            onClick={onClose}
                            className="flex-1 bg-muted text-foreground py-2 px-4 rounded-sm hover:bg-gray-300 transition-colors"
                        >
                            Cancel
                        </button>
                        <button
...
export const PaymentModal: React.FC<PaymentModalProps> = ({ isOpen, onClose, invoice, onSubmit, availableCredit = 0, isDebitNote = false }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex justify-center items-center z-[2000]">
            <div className="bg-card rounded-sm shadow-xl p-6 w-full max-w-md">
                <h2 className="text-2xl font-bold mb-2 text-foreground">Settle Payment</h2>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-4">
                        <label htmlFor="amount" className="block text-sm font-medium text-foreground">Amount to Settle</label>
                        <input
                            type="number"
                            id="amount"
                            value={amount}
                            onChange={(e) => setAmount(e.target.value)}
                            className="mt-1 block w-full rounded-sm border-border shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border"
                            required
                        />
                    </div>
                    <div className="mb-6">
                        <label htmlFor="method" className="block text-sm font-medium text-foreground">Payment Method</label>
                        <select
                            id="method"
                            value={method}
                    ...
                    {method === 'PDC' && (
                        <div className="mb-6 bg-muted p-3 rounded-sm border border-border">
                            <div className="mb-4">
                                <label className="block text-xs font-bold text-foreground uppercase tracking-wide">Cheque Number</label>
                                <input
                                    type="text"
                                    value={chequeNumber}
                                    onChange={(e) => setChequeNumber(e.target.value)}
                                    className="mt-1 block w-full rounded-sm border-border shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border text-sm"
                                    placeholder="Enter cheque number"
                                    required
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-foreground uppercase tracking-wide">Cheque Date</label>
                                <input
                                    type="date"
                                    value={chequeDate}
                    ...
                    <div className="flex justify-end gap-3 mt-2">
                        <button type="button" onClick={onClose} className="px-4 py-2.5 rounded-sm bg-muted text-foreground font-bold hover:bg-slate-300 text-sm">Cancel</button>
